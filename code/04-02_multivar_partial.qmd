...
- Clustering - 
    - **k-means**, c-means, x-means  - #uses distance-based algorithms
    - Non-numerical multidimensional scaling  
    - nearest neighbor  
  
- Relationship between variables / Prediction  - establishes relationships unlike the previous 2 types
...
- Based on Y (outcome) existence:
    - Supervised  - if you have Y variable/s in the formula , then, it is supervises; you've given the label, an answer to x -variable
    - Unsupervised - no Y variable, no label. just x-variables, asking for groups that doesn't really explain a Y variable or a label

- If Y exists, based on its type:  
    - Categorical: classification  - high, low, medium yield - classify the variable not based on the actual number of yield but based on the classification classes
...
ggsave("../output/corrmat.png",
       ...
       bg = "white")
```
* r value in the cells; positive corr = red , purple = nega corr, 
...
weather %>%
  ...
  mutate(r2 = map_dbl(data,
                  ~lm(strength_gtex ~ value,
                      data = .x) %>%
                    glance(.) %>% #glance - creates a whole bunch of matrix
                    pull(r.squared)
                  )) %>%
  arrange(desc(r2))
...
lm(strength_gtex ~ mean_dayl.s_Apr + 
                   sum_prcp.mm_Oct,
   data = weather) %>%
  vif() #vif = 1 thus, no multicollinearity
...
lm(strength_gtex ~ mean_dayl.s_Apr + 
                  mean_dayl.s_May,
   data = weather) %>%
  vif() #severe multicollinearity, thus, impacting the estimates, errors, and interpretation
...
Thus, our numerical predictor variables need to be normalized (center and scale) before starting our multivariate analysis.  # so they have mean 0 and sd = 0
...
# PCA  
Principal component analysis (PCA) is a dimensionality reduction approach that accommodates only numerical variables.\

* PCA looks for variables that are highly correlated, and give predictors
* PCAs are orthogonal, idependent, not multicolinear
...
- Is an unsupervised analysis (no outcome)  ; thus, no y variable
- Only takes predictors  ; not predicted variable
...
```
PCA
* 1. standardize data - center = 0

* 2. covariance matrix

* 3. eigenvector - higher, more variability; length of arrow, is the eigenvector

The first PC, PC1 = will explain most variability; largest dimension

PC2 = next dimension that has 2nd most variability, and so on
...
summary(mod_pca) # prop of variance, PC1 -explains 35% of variability

```

* PC is not a variable selection tool by itself
...
Which variables contributed most to PC1, regardless of direction? mean_dayl.s_Jan, mean_dayl.s_Dec

* variables that have the highest loadings are related to mean_dayl.s from Jan - Oct ; and lowest loadings are mean_dayl.s from Apr - September
...
fviz_contrib(mod_pca,
             ...
             axes = 1)
```
* dotted red line = overall average loading
...
fviz_pca_ind(mod_pca,
             label = F)
```
  each PC = each variable * to weight of coefficient; added together for all variables,
...
```{r pca regression}
# Checking VIF
vif(lm_pca) # all 1 coz they are orthogonal, independent
```
...
ggplot(weather_postpca, 
  ...
  geom_point()+
  geom_smooth(method = "lm") #PC 2 positive relationship; slope is significant based on lm summary
...
ggplot(weather_postpca, 
  ...
  geom_point()+
  geom_smooth(method = "lm")  #PC 3 negative relationship with strength
...
```
* choose a number of k where you see an elbow, so, in this case, between 3 and 5 clusters
* elbow method
...
* we want the silhouette width to be as high as possible
...
weather %>%
  ...
  pivot_longer(!c(year,site,cluster)) %>% #except year, site, and cluster
